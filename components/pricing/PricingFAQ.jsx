const FAQS = [
  ...
  },
  {
    ...
    a: 'By default, your agent continues at the overage rate so the task completes. If you’ve set a hard cap, the agent pauses and notifies you. You can resume by topping up or waiting until your next cycle.',
  },
  {
    q: 'Why is the first month cheaper?',
    a: 'We want you to try the product on real work, not just a demo. The first-month price gives you meaningful runway to see if it fits. After that, the regular rate kicks in — no surprise jumps, no auto-locked contracts.',
  },
  {
    q: 'Can I share my plan with my team?',
    a: 'The Pro and Max plans are single-user. For team use, the Enterprise plan supports multiple seats, shared resources, and role-based access. Custom team pricing is available — talk to sales.',
  },
  {
  ...
  },
  {
    ...
    a: 'The Free plan is open-ended — use it as long as you like. The first-month discount on paid plans is effectively a try-it-out price. If a paid plan doesn’t work for you in the first 14 days, we’ll refund it on request, no questions asked.',
  },
  {
    q: 'Can I run an agent 24/7?',
    a: 'Yes. Agents run continuously within your plan’s resource limits — tokens are the thing to watch for long-running tasks. The dashboard shows live consumption so you can tune behavior.',
  },
  {
    q: 'How does fine-tuning work? Do I pay for GPUs?',
    a: 'You bring your own cloud (AWS, GCP, Lambda, RunPod, etc.) via dstack or skypilot. Your cloud provider bills you directly for GPU time. We orchestrate the training run, manage checkpoints, and track lineage in the hub. No GPU markup from us.',
  },
  {
  ...
  },
  {
    ...
    a: 'Yes, on Pro and Max. You deploy via your own cloud (dstack/skypilot), and we orchestrate the endpoint. Your agents can call your custom models directly.',
  },
  {
    q: 'What’s the hub for?',
    a: 'A git-backed registry for your models, datasets, and checkpoints — like a private Hugging Face Hub. Pull public models, push your own, version everything. Free for read-only access; private storage by tier.',
  },
  {
      ...
      <div className="max-w-container mx-auto px-8 grid gap-12 lg:gap-16 grid-cols-1 lg:[grid-template-columns:minmax(0,360px)_1fr]">
        <div>
          ...
          <h2 className="reveal mt-3.5 font-serif font-semibold leading-[1.02] tracking-hl-tight text-[36px] sm:text-[44px] lg:text-[52px] text-brand-aubergine">
            Questions, answered{' '}
            <span className="italic font-medium">in plain language</span>.
          </h2>
          ...
          {FAQS.map((item) => (
            ...
            >
              <summary className="flex items-baseline gap-4 cursor-pointer list-none">
                ...
                >
                  <svg viewBox="0 0 14 14" className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
